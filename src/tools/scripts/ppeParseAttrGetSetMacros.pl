...
my $ocmbFile = $ARGV[0];
$ocmbFile .= "/";
$ocmbFile .= "proc_sbe_fixed_ocmb_chip.H";
open(OCMBFILE, ">", $ocmbFile);

my $pmicFile = $ARGV[0];
$pmicFile .= "/";
$pmicFile .= "proc_sbe_fixed_pmic.H";
open(PMICFILE, ">", $pmicFile);

my $gi2cFile = $ARGV[0];
$gi2cFile .= "/";
$gi2cFile .= "proc_sbe_fixed_gi2c.H";
open(GI2CFILE, ">", $gi2cFile);
...
foreach my $entr (@{$entries->{entry}}) {
    ...
    foreach my $argnum (2 .. $#ARGV)
    {
        ...
        foreach my $attr (@{$attributes->{attribute}})
        {
            ...
            } elsif($attr->{targetType} eq "TARGET_TYPE_OCMB_CHIP") {

                push(@attrOcmbChipIds, $attr);

            } elsif($attr->{targetType} eq "TARGET_TYPE_PMIC") {

                push(@attrPMICIds, $attr);

            } elsif($attr->{targetType} eq "TARGET_TYPE_GENERICI2CSLAVE") {

                push(@attrGI2CIds, $attr);
...
print OCMBFILE "// proc_sbe_fixed_ocmb_chip.H\n";
print OCMBFILE "// This file is generated by perl script ppeParseAttrGetSetMacros.pl\n\n";
print OCMBFILE "#ifndef __PROC_SBE_FIXED_OCMB_CHIP_H__\n";
print OCMBFILE "#define __PROC_SBE_FIXED_OCMB_CHIP_H__\n\n";
foreach my $attr (@attrOcmbChipIds)
{

    my $value = uc $attr->{valueType};
    print OCMBFILE "PROC_SBE_FIXED_$value($attr->{id});\n"


}
print OCMBFILE "\n#endif  // __PROC_SBE_FIXED_OCMB_CHIP_H__";

print PMICFILE "// proc_sbe_fixed_pmic.H\n";
print PMICFILE "// This file is generated by perl script ppeParseAttrGetSetMacros.pl\n\n";
print PMICFILE "#ifndef __PROC_SBE_FIXED_PMIC_H__\n";
print PMICFILE "#define __PROC_SBE_FIXED_PMIC_H__\n\n";
foreach my $attr (@attrPMICIds)
{

    my $value = uc $attr->{valueType};
    print PMICFILE "PROC_SBE_FIXED_$value($attr->{id});\n"


}
print PMICFILE "\n#endif  // __PROC_SBE_FIXED_PMIC_H__";

print GI2CFILE "// proc_sbe_fixed_gi2c.H\n";
print GI2CFILE "// This file is generated by perl script ppeParseAttrGetSetMacros.pl\n\n";
print GI2CFILE "#ifndef __PROC_SBE_FIXED_GI2C_H__\n";
print GI2CFILE "#define __PROC_SBE_FIXED_GI2C_H__\n\n";
foreach my $attr (@attrGI2CIds)
{

    my $value = uc $attr->{valueType};
    print GI2CFILE "PROC_SBE_FIXED_$value($attr->{id});\n"


}
print GI2CFILE "\n#endif  // __PROC_SBE_FIXED_GI2C_H__";
