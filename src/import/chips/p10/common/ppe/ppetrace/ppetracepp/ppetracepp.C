...
/*                                                                        */
/* $Source: src/import/chips/p10/common/ppe/ppetrace/ppetracepp/ppetracepp.C $ */
/*                                                                        */
...
bool chop_up_line(string& in_line, string& prefix, string& strings, string& salt, string& suffix)
{
    ...
    {
        return(false);    //non-whitespace?
    }

    if ((pos2 != pos3) && (line.find_first_not_of(" \t", pos2, (pos3 - pos2)) != string::npos))
    {
        return(false);    //non-whitespace?
    }
    ...
    while(!line.empty())
    {
        ...
        if ((pos1 == string::npos) || ((pos != string::npos) && (pos < pos1)))
        {
            break;    // found comma before next quote
        }
...
int get_format_string(const string& in_str, string& format)
{
    ...
    while(!str.empty())
    {
        ...
        if (pos == (str.length() - 1))
        {
            break;    // last char in string?  just skip it
        }
...
int main(int argc, char** argv)
{
    ...
    while (!feof(CPP))
    {
        if (fgets(buf, MAX_BUFFER, CPP) != NULL)
        {
            ...
            if (DEBUGIN)
            {
                fprintf(DEBUGIN, "%s", oldline.c_str());
            }
