    ...
    const uint32_t i_mem_flags)
{
    ...
    FAPI_ASSERT(((i_mem_flags & fapi2::SBE_MEM_ACCESS_FLAGS_TARGET_PROC) == fapi2::SBE_MEM_ACCESS_FLAGS_TARGET_PROC) &&
                ...
                fapi2::P10_PUTMEMPROC_INVALID_FLAGS().
                set_TARGET(i_target).
    ...
    FAPI_ASSERT(((i_mem_flags & fapi2::SBE_MEM_ACCESS_FLAGS_ECC_OVERRIDE) == 0) &&
                ...
                fapi2::P10_PUTMEMPROC_UNSUPPORTED_FLAGS().
                set_TARGET(i_target).
                set_ADDRESS(i_address).
                set_BYTES(i_bytes).
                set_FLAGS(i_mem_flags).
    ...
    {
        l_flags.setOperationType(adu_operationFlag::CACHE_INHIBIT);

        if (i_bytes == 4)
        {
            l_flags.setTransactionSize(adu_operationFlag::TSIZE_4);
        }
        else if (i_bytes == 2)
        {
            l_flags.setTransactionSize(adu_operationFlag::TSIZE_2);
        }
        else if (i_bytes == 1)
        {
            l_flags.setTransactionSize(adu_operationFlag::TSIZE_1);
        }
        else if (i_bytes == 8)
        {
            l_flags.setTransactionSize(adu_operationFlag::TSIZE_8);
        }
        else
        {
            FAPI_ASSERT(false,
                        fapi2::P10_PUTMEMPROC_INVALID_SIZE_CI().
                        set_TARGET(i_target).
                        set_ADDRESS(i_address).
                        set_BYTES(i_bytes).
                        set_FLAGS(i_mem_flags),
    ...
    else
    {
        ...
        {
            FAPI_ASSERT(false,
                        fapi2::P10_PUTMEMPROC_INVALID_SIZE_DMA().
                        set_TARGET(i_target).
                        set_ADDRESS(i_address).
                        set_BYTES(i_bytes).
                        set_FLAGS(i_mem_flags),
