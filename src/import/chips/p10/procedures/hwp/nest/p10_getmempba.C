...
/*                                                                        */
/* $Source: src/import/chips/p10/procedures/hwp/nest/p10_getmempba.C $    */
/*                                                                        */
...
/* IBM_PROLOG_END_TAG                                                     */
///
/// @file p10_getmempba.C
/// @brief Invoke PBA getmem chip-op.
///
...
fapi2::ReturnCode p10_getmempba(
    ...
    const uint32_t i_mem_flags)
{
    ...
    const fapi2::Target<fapi2::TARGET_TYPE_CORE> l_core_target;
    ...
    while (l_target_address < l_end_address)
    {
        ...
        FAPI_TRY(p10_pba_setup(i_target,
                               l_core_target, // Not used for read
                               l_target_address,
                               true, // Read
                               l_flags.setFlag(),
        ...
        while (l_granules_before_setup && (l_target_address < l_end_address))
        {
            ...
            // NOTE: SBE environment does not support memset and therefore must use
            // compiler assisted initialization for dynamically sized memory, whereas some other
            // environments do not support that ability.
#ifndef __PPE__
            ...
            // Load l_granuleSize data into array.
            for (uint32_t ii = 0; ii < l_granuleSize; ii++)
            {
                o_data[(l_granule * l_granuleSize + ii)] = l_data[ii] ;
            }
