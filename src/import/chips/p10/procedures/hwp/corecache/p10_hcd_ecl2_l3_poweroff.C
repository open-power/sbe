/* IBM_PROLOG_BEGIN_TAG                                                   */
/* This is an automatically generated prolog.                             */
/*                                                                        */
/* $Source: src/import/chips/p10/procedures/hwp/corecache/p10_hcd_ecl2_l3_poweroff.C $ */
/*                                                                        */
/* OpenPOWER sbe Project                                                  */
/*                                                                        */
/* Contributors Listed Below - COPYRIGHT 2020                             */
/* [+] International Business Machines Corp.                              */
/*                                                                        */
/*                                                                        */
/* Licensed under the Apache License, Version 2.0 (the "License");        */
/* you may not use this file except in compliance with the License.       */
/* You may obtain a copy of the License at                                */
/*                                                                        */
/*     http://www.apache.org/licenses/LICENSE-2.0                         */
/*                                                                        */
/* Unless required by applicable law or agreed to in writing, software    */
/* distributed under the License is distributed on an "AS IS" BASIS,      */
/* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or        */
/* implied. See the License for the specific language governing           */
/* permissions and limitations under the License.                         */
/*                                                                        */
/* IBM_PROLOG_END_TAG                                                     */

///
/// @file  p10_hcd_core_poweroff.C
/// @brief Power off Core+L2 VDD+VCS PFET headers
///

// *HWP HWP Owner          : David Du               <daviddu@us.ibm.com>
// *HWP Backup HWP Owner   : Greg Still             <stillgs@us.ibm.com>
// *HWP FW Owner           : Prasad Brahmasamurdra  <prasadbgr@in.ibm.com>
// *HWP Team               : PM
// *HWP Consumed by        : SBE:QME
// *HWP Level              : 2


//------------------------------------------------------------------------------
// Includes
//------------------------------------------------------------------------------
#include "p10_hcd_ecl2_l3_poweroff.H"
#include "p10_hcd_core_poweroff.H"
#include "p10_hcd_cache_poweroff.H"
#include "p10_hcd_mma_poweroff.H"
#include "p10_hcd_corecache_power_control.H"
#include "p10_hcd_common.H"

#include "p10_scom_eq.H"
#include "p10_scom_c.H"
using namespace scomt::eq;
using namespace scomt::c;

//------------------------------------------------------------------------------
// Constant Definitions
//------------------------------------------------------------------------------


//------------------------------------------------------------------------------
// Procedure: p10_hcd_ecl2_l3_poweroff
//------------------------------------------------------------------------------


fapi2::ReturnCode
p10_hcd_ecl2_l3_poweroff(
    const fapi2::Target < fapi2::TARGET_TYPE_CORE | fapi2::TARGET_TYPE_MULTICAST, fapi2::MULTICAST_AND > & i_target)
{
    fapi2::buffer<uint64_t> l_data64;

    FAPI_INF(">>p10_hcd_ecl2_l3_poweroff");

    // Core + L2 + MMA poweroff
    FAPI_TRY( p10_hcd_core_poweroff( i_target ) );

    // L3 poweroff
    FAPI_TRY( p10_hcd_cache_poweroff( i_target ) );

    //If we are in success in the above two procedure, then we need to update
    //STOP GATED and ACT_STOP_LVL bits in SSH_SRC register

    FAPI_TRY( fapi2::putScom( i_target, QME_SSH_SRC, BIT64(QME_SSH_SRC_STOP_GATED)
                              | BITS64(QME_SSH_SRC_ACT_STOP_LEVEL, QME_SSH_SRC_ACT_STOP_LEVEL_LEN)) );

    //Clear PM Block interrupts
    FAPI_TRY( fapi2::putScom( i_target, QME_SCSR_WO_CLEAR, BIT64(QME_SCSR_ASSERT_PM_BLOCK_INTR)) );

fapi_try_exit:

    FAPI_INF("<<p10_hcd_ecl2_l3_poweroff");

    return fapi2::current_err;

}
