    ...
    const fapi2::Target<fapi2::TARGET_TYPE_PROC_CHIP>& i_target)
{
    ...
    l_data64.flush<0>();
    FAPI_TRY(fapi2::putScom(i_target, PU_OCB_OCI_OIRR0A_SCOM, l_data64),
             "**** ERROR : Unexpected error encountered in write to OCC "
             "Interrupt 0 Route A Register (OIRR0A)");

    FAPI_TRY(fapi2::putScom(i_target, PU_OCB_OCI_OIRR0B_SCOM, l_data64),
             "**** ERROR : Unexpected error encountered in write to OCC "
             "Interrupt 0 Route B Register (OIRR0A)");

    FAPI_TRY(fapi2::putScom(i_target, PU_OCB_OCI_OIRR0C_SCOM, l_data64),
             "**** ERROR : Unexpected error encountered in write to OCC "
             "Interrupt 0 Route C Register (OIRR0A)");

    FAPI_TRY(fapi2::putScom(i_target, PU_OCB_OCI_OIRR1A_SCOM, l_data64),
             "**** ERROR : Unexpected error encountered in write to OCC "
             "Interrupt 1 Route A Register (OIRR1A)");

    FAPI_TRY(fapi2::putScom(i_target, PU_OCB_OCI_OIRR1B_SCOM, l_data64),
             "**** ERROR : Unexpected error encountered in write to OCC "
             "Interrupt 1 Route B Register (OIRR1B)");

    FAPI_TRY(fapi2::putScom(i_target, PU_OCB_OCI_OIRR1C_SCOM, l_data64),
             "**** ERROR : Unexpected error encountered in write to OCC "
    ...
    // hardware procedures.
    FAPI_TRY(fapi2::getScom(i_target, PU_PBAMODE_SCOM, l_data64),
             "**** ERROR : Failed to fetch PBA mode control status");
    l_data64.clearBit<PU_PBAMODE_EN_MARKER_ACK>();
    ...
    FAPI_TRY(fapi2::putScom(i_target, PU_OCB_PIB_OSTOESR, l_data64),
    ...
    FAPI_TRY(fapi2::getScom(i_target, PU_OCB_OCI_OCCHBR_SCOM, l_data64),
             "**** ERROR : Failed to read OCBHBR");
    l_data64.clearBit<PU_OCB_OCI_OCCHBR_OCC_HEARTBEAT_EN>();
    FAPI_TRY(fapi2::putScom(i_target, PU_OCB_OCI_OCCHBR_SCOM, l_data64),
