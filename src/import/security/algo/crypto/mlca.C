...
/*                                                                        */
/* $Source: src/import/security/algo/crypto/mlca.C $                      */
/*                                                                        */
...
/*----------------------------------------------------------------------
 * (C) COPYRIGHT INTERNATIONAL BUSINESS MACHINES CORPORATION 2018-2023
 *                        ALL RIGHTS RESERVED
 *                       IBM Research - Zurich
 *----------------------------------------------------------------------
 *  Author: Visegrady, Tamas (tvi@zurich.ibm.com)
 *          Hess, Basil (bhe@zurich.ibm.com)
 *----------------------------------------------------------------------*/

/* cross-compliance against ref.impl verified at:
 *   9dddb2a0537734e749ec2c8d4f952cb90cd9e67b  2020-07-21
 *
 ...
 *
 *   round2 and round3
 *   Kyb III   Kyber768  (Kyber-3/3)
 *   Kyb V     Kyber1024 (Kyber-4/4)
 *
 * define
 *    USE_STATIC_MLCA  -- make public functions static; suitable for
 *                     -- single-file distribution
 *    NO_RNG           -- to use a fast, _non-cryptographic_ [non-]RNG
 *                     -- we have instrumented ref.impl. data, so this
 *                     -- version can be used for KATs
 *    I390QS           -- configure for the System Z, Onload I/O subsystem
 *    SYS_CLIC         -- use other features of the IBM Clic crypto provider
 *
 ...
 *    NO_CRYSTALS_CIP  -- remove encryption (Kyber)
 ...
 *    SKIP_DIL         -- skip Dilithium-related calls
 *    SKIP_DIL2, SKIP_DIL3, SKIL_DIL4, SKIP_DIL5
 *                     -- ...Dilithium of specific categories...
 *    SKIP_DIL_R2      -- skip round2 Dilithium functionality
 *    SKIP_DIL_R3      -- skip round3 ...
 *    SKIP_KYB or SKIP_KYBER        -- ...Kyber...
 *    SKIP_KYB768, SKIP_KYBER768    -- skip specific Kyber category
 *    SKIP_KYB1024, SKIP_KYBER1024  -- ...specific categories...
 *    SKIP_CRYPT       -- skip IND/CPA (en/decrypt) direct calls; only KEM
 *    SKIP_KEM         -- skip key agreement
 *    SKIP_FUNC        -- test only key.gen and serialization, skips functional
 *                     -- tests such as sign+verify cycles
 *    SKIP_MUCH        -- keep the number of multi-iteration tests to
 *                     -- 'reasonably small' iteration count
 *    CHAIN            -- include chained KATs where deterministic algs
 *                     -- (or those made deterministic through NO_RNG)
 *                     -- have chained, compact known-answer sequences
 *    ITERATE          -- nr. of iterations for each main type of test
 *                     -- set 0 for endless chains
 *
 * markers are placed to delimit certain parts for automated
 * processing (structured comments):
 *    MARKER: nnn     with 'nnn' selecting section:
 *       nnn: TOP             -- entire file
 *            SYS.INCLUDE     -- include std... (any of the system headers)
 *            KECCAK          -- SHA-3
 *            RNG             -- random-number generator
 * each marker includes its matching "/MARKER: ..." counterpart;
...
// 1. remove first part
// 2. add defines
// 3. add __mlca_generate and mlca_generate without first argument, same for other functions
// 4. Remove !pKey argument checks
...
/* include guards, not a typo; assume single-file .c may be included */

// /* MARKER: SYS.INCLUDE  [DO NOT MODIFY] */
#include <stdint.h>
#if defined(SYS_Z14)
    #include <vecintrin.h>                /* SIMD polyvec */
#endif
#include <stddef.h>
#ifdef __PPE__
...
/*------------------------------------*/
static __INLINE__ uint64_t MSBF8_READ(const void* p)
{
    const unsigned char* pb = (const unsigned char*)p;

    return (((uint32_t)pb[7]) | (((uint32_t)pb[6]) << 8) | (((uint32_t)pb[5]) << 16) |
            (((uint32_t)pb[4]) << 24) | (((uint64_t)pb[3]) << 32) | (((uint64_t)pb[2]) << 40) |
            (((uint64_t)pb[1]) << 48) | (((uint64_t)pb[0]) << 56));
}
 ...
 * recent gcc/clang and some xlc's tend to do so
 */
static __INLINE__ void MSBF8_WRITE(void* p, uint64_t v)
{
    unsigned char* pb = (unsigned char*)p;
...
//--------------------------------------
static __INLINE__ void* crs_memclr(void* s, size_t bytes)
{
    if ( s && bytes )
    {
        volatile unsigned char* tmp = (volatile unsigned char*)s;
 ...
 * tolerates NULL val
 */
CRS_STATIC inline uint64_t cu_readuint(const char* val, size_t vbytes)
{
    ...
    if ( val )
    {
        ...
        if ( (!vbytes || (2 < vbytes)) && ('0' == val[0]) && ('x' == val[1]) )
        {
            base = 16;
            val += 2;
...
//--------------------------------------
static __INLINE__ void cu_hexprint(const char* prefix, const void* data, size_t dbytes)
{
...
//--------------------------------------
static __INLINE__ long crs_reportrc(const char* msg, long rc)
{
    fflush(stdout);
...
/*--------------------------------------
 * XORs together [alleged PRF] data into a 64-bit checksum
 */
ATTR_PURE__
static __INLINE__ uint64_t crs_sum64(const void* buf, size_t bytes)
{
    uint64_t x = 0;
...
/* note: needs replacement for 8+3 on z/VM */
...
{
    // return mlca_randombytes((mlca_random_t *)rng, r, rbytes);
    return 1;
}

void MEMSET0_STRICT(void* mem, size_t size)
{
 ...
 **************************************************/
CRS_STATIC void kyb__basemul(int16_t r[2], const int16_t a[2], const int16_t b[2], int16_t zeta)
{
...
CRS_STATIC void r3_kyb__basemul(int16_t r[2], const int16_t a[2], const int16_t b[2], int16_t zeta)
{
...
CRS_STATIC void shake128_stream_init(Keccak_state* state, const uint8_t seed[DIL_SEEDBYTES],
                                     uint16_t nonce)
{
    uint8_t t[2];
...
/*------------------------------------*/
CRS_STATIC void shake256_stream_init(Keccak_state* state, const uint8_t seed[DIL_CRHBYTES],
                                     uint16_t nonce)
{
    uint8_t t[2];
 ...
 */
CRS_STATIC void shake256_stream_init_dilr3(Keccak_state* state, const uint8_t seed[DIL_R3_CRHBYTES],
        uint16_t nonce)
{
    uint8_t t[2];
 ...
 **************************************************/
CRS_STATIC unsigned int rej_uniform(uint32_t* a, unsigned int len, const uint8_t* buf,
                                    unsigned int buflen)
{
...
CRS_STATIC void poly_uniform(poly* a, const uint8_t seed[DIL_SEEDBYTES], uint16_t nonce)
{
 ...
 * random bytes were given.
 **************************************************/
CRS_STATIC unsigned int rej_eta(uint32_t* a, unsigned int len, const uint8_t* buf,
...
CRS_STATIC void poly_uniform_eta(poly* a, const uint8_t seed[DIL_SEEDBYTES], unsigned int eta,
                                 uint16_t nonce)
{
 ...
 * random bytes were given.
 **************************************************/
CRS_STATIC unsigned int rej_gamma1m1(uint32_t* a, unsigned int len, const uint8_t* buf,
                                     unsigned int buflen)
{
#if DIL_GAMMA1 > (1 << 19)
...
CRS_STATIC void poly_uniform_gamma1m1(poly* a, const uint8_t seed[DIL_CRHBYTES], uint16_t nonce)
{
    unsigned int    i, ctr, off;
 ...
 **************************************************/
CRS_STATIC void spoly_pointwise_montgomery(spoly* c, const spoly* a, const spoly* b)
{
    unsigned int i;
 ...
 *              - const spoly *a: pointer to input polynomial
 **************************************************/
CRS_STATIC void spoly_decompose(spoly* a1, spoly* a0, const spoly* a, unsigned int dil_k)
{
    unsigned int i;
 ...
 * Returns number of 1 bits.
 **************************************************/
CRS_STATIC unsigned int spoly_make_hint(spoly* h, const spoly* a0, const spoly* a1,
                                        unsigned int dil_k)
{
    unsigned int i, s = 0;
 ...
 **************************************************/
CRS_STATIC void spoly_use_hint(spoly* b, const spoly* a, const spoly* h, unsigned int dil_k)
{
    unsigned int i;
 ...
 * random bytes were given.
 **************************************************/
CRS_STATIC unsigned int rej_s_uniform(int32_t* a, unsigned int len, const uint8_t* buf,
                                      unsigned int buflen)
{
    unsigned int ctr, pos;
    uint32_t     t;
...
CRS_STATIC void spoly_uniform(spoly* a, const uint8_t seed[DIL_SEEDBYTES], uint16_t nonce)
{
    unsigned int    buflen = POLY_UNIFORM_NBLOCKS * DIL_STREAM128_BLOCKBYTES;
    uint8_t         buf[POLY_UNIFORM_NBLOCKS * DIL_STREAM128_BLOCKBYTES + 2];
    unsigned int    i, ctr, off;
 ...
 * random bytes were given.
 **************************************************/
CRS_STATIC unsigned int rej_s_eta(int32_t* a, unsigned int len, const uint8_t* buf,
                                  unsigned int buflen, unsigned int eta)
{
    unsigned int ctr, pos;
    uint32_t     t0, t1;
...
/*---------------------------------------------------
 */
CRS_STATIC void spoly_uniform_eta(spoly* a, const uint8_t seed[DIL_R3_CRHBYTES], uint16_t nonce,
                                  unsigned int eta)
{
...
CRS_STATIC void spoly_uniform_gamma1(spoly* a, const uint8_t seed[DIL_R3_CRHBYTES], uint16_t nonce,
                                     unsigned int dil_k)
{
 ...
 **************************************************/
CRS_STATIC void spoly_challenge(spoly* c, const uint8_t seed[DIL_SEEDBYTES], unsigned int dil_k)
{
...
/**************************************/
CRS_STATIC void kyber_shake256_prf(uint8_t* out, size_t outlen, const uint8_t key[KYB_SYMBYTES],
                                   uint8_t nonce)
{
    ...
    crs_memclr(extkey, sizeof(extkey));
}
 ...
 **************************************************/
CRS_STATIC void kpoly_getnoise(kpoly* r, const uint8_t seed[KYB_SYMBYTES], uint8_t nonce)
{
    ...
    crs_memclr(buf, sizeof(buf));
}

CRS_STATIC void r3_kpoly_getnoise_eta1(kpoly* r, const uint8_t seed[KYB_SYMBYTES], uint8_t nonce)
{
    uint8_t buf[KYB_ETA * KYB_N / 4] CRS_SENSITIVE;
...
CRS_STATIC void r3_kpoly_getnoise_eta2(kpoly* r, const uint8_t seed[KYB_SYMBYTES], uint8_t nonce)
{
 ...
 **************************************************/
CRS_STATIC void kpoly_basemul_montgomery(kpoly* r, const kpoly* a, const kpoly* b)
{
    unsigned int i;
...
CRS_STATIC void r3_kpoly_basemul_montgomery(kpoly* r, const kpoly* a, const kpoly* b)
{
    unsigned int i;
...
CRS_STATIC size_t dil_prv_wirebytes(unsigned int k, unsigned int l, unsigned int round)
{
    if ( !l )
    {
        l = k - 1;
...
//--------------------------------------
// raw bytecount, excluding ASN framing
CRS_STATIC size_t dil_pub_wirebytes(unsigned int k, unsigned int l, unsigned int round)
{
    if ( !l )
    {
        l = k - 1;
...
//
CRS_STATIC size_t dil_signature_bytes(unsigned int k, unsigned int l, unsigned int round)
{
    switch ( (round << 16) | (k << 4) | l )
    {
        case 0x20054:
 ...
 */
CRS_STATIC size_t crs_oid2wire(unsigned char* wire, size_t wbytes, unsigned int type)
{
    uint32_t tail = 0;
    size_t   wr;
...
//
CRS_STATIC size_t crs_asn_bitstr(unsigned char* wire, size_t wbytes, size_t bstring_net_bytes)
{
...
//--------------------------------------
CRS_STATIC size_t crs_asn_something(unsigned char* wire, size_t wbytes, size_t seq_net_bytes,
                                    unsigned char tag)
{
...
/*------------------------------------*/
CRS_STATIC size_t crs_asn_sequence(unsigned char* wire, size_t wbytes, size_t seq_net_bytes)
{
    return crs_asn_something(wire, wbytes, seq_net_bytes, CRS__ASN1_SEQUENCE);
}

/*------------------------------------*/
CRS_STATIC size_t crs_asn_octetstr(unsigned char* wire, size_t wbytes, size_t seq_net_bytes)
{
 ...
 */
CRS_STATIC size_t crs_asn_int(unsigned char* wire, size_t wbytes, unsigned char i)
{
    if ( wire && (wbytes >= 3) )
    {
 ...
 */
#if defined(NO_CRYSTALS_R2_5)
CRS_STATIC size_t crs_asn_expl(unsigned char* wire, size_t wbytes, size_t netbytes,
                               unsigned char class)
{
    if ( wire && (wbytes >= 4) )
    {
 ...
 */
CRS_STATIC size_t crs_asn_oid_extd(unsigned char* wire, size_t wbytes, unsigned int type)
{
    size_t oidb;
 ...
 */
CRS_STATIC size_t is_bitstr_at(const unsigned char* asn, size_t abytes, size_t offs,
                               size_t net_bytes)
{
 ...
 * TODO: support prv-to-public serialization
 */
#if defined(NO_CRYSTALS_R2_5)
CRS_STATIC size_t dil_prv2wire(unsigned char* wire, size_t wbytes, const unsigned char* prv,
                               ...
                               size_t pubbytes, const unsigned char* algid, size_t ibytes)
{
    ...
    uint32_t     wtype = 0; /* type retrieved from raw input */

    /* set wtype >0 to force subsequent read-and-check */
    ...
    if ( wtype )
    {
        wtype = MSBF4_READ(prv + prvbytes - CRS_WTYPE_BYTES);
 ...
 * TODO: support prv-to-public serialization
 */
CRS_STATIC size_t dil_pub2wire(unsigned char* wire, size_t wbytes, const unsigned char* pub,
 ...
 */
#if defined(NO_CRYSTALS_R2_5)
CRS_STATIC size_t kyb_prv2wire(unsigned char* wire, size_t wbytes, const unsigned char* prv,
                               size_t pbytes, const unsigned char* algid, size_t ibytes)
{
    ...
    if ( wtype )
    {
        wtype = MSBF4_READ(prv + pbytes - CRS_WTYPE_BYTES);
...
/*--------------------------------------
    ...
    *
    * TODO: proper DER deframe; matching hardwired templates
    */
CRS_STATIC size_t kyb_wire2prv(unsigned char* prv, size_t pbytes, unsigned int* type,
                               ...
                               size_t ibytes)
{
    ...
    do
    {
        ...
        /* TODO: MUST be replaced by BER parsing
    ...
    /* opportunistic: write trailing type as BE32
        ...
        */
    if ( wtype && prv && (wr > CRS__SZ_MAXERROR) && (wr + CRS_WTYPE_BYTES <= wbytes) )
    {
...
/*--------------------------------------
    ...
    */
CRS_STATIC size_t kyb_wire2pub(unsigned char* pub, size_t pbytes, unsigned int* type,
                               const unsigned char* wire, size_t wbytes, const unsigned char* algid,
                               size_t ibytes)
{
...
//--------------------------------------
CRS_STATIC size_t kyb_pub2wire(unsigned char* wire, size_t wbytes, const unsigned char* pub,
                               size_t pbytes, const unsigned char* algid, size_t ibytes)
{
    size_t       wr = 0, offs = 0;
    unsigned int type  = 0;
    ...
    switch ( pub ? pbytes : 0 )
    {
        case KYB_PUB3_BYTES + CRS_WTYPE_BYTES:
            wtype = 3;
            break;

        case KYB_PUB4_BYTES + CRS_WTYPE_BYTES:
            wtype = 4;
            break;
...
/*--------------------------------------
    ...
    * TODO: proper DER deframe
    */
CRS_STATIC size_t dil_wire2pub(unsigned char* pub, size_t pbytes, unsigned int* type,
                               const unsigned char* wire, size_t wbytes, const unsigned char* algid,
                               size_t ibytes)
{
...
*
* TODO: proper DER deframe; currently matching hardwired templates
* see dil__sections[]
*/
CRS_STATIC size_t dil_wire2prv(unsigned char* prv, size_t pbytes, unsigned int* type,
                               const unsigned char* wire, size_t wbytes, const unsigned char* algid,
                               size_t ibytes)
{
    ...
    /* TODO: shortcut: reject partial structs; import+recreate them */
    ...
    /* TODO: rework around ASN-array importer */
    do
    {
        ...
        {
            if ( !s.t1b )                 /* TODO: reconstruct missing PUB from PRV */
            {
...
/**/
CRS_STATIC void kpolyvec_compress(unsigned char* r, size_t rbytes, kpolyvec_max* a,
                                  unsigned int kyb_k);
CRS_STATIC void r3_kpolyvec_compress(unsigned char* r, size_t rbytes, kpolyvec_max* a,
                                     unsigned int kyb_k);
/**/
CRS_STATIC void kpolyvec_decompress(kpolyvec_max* r, unsigned int kyb_k, const unsigned char* a,
                                    size_t abytes);
CRS_STATIC void r3_kpolyvec_decompress(kpolyvec_max* r, unsigned int kyb_k, const unsigned char* a,
                                       size_t abytes);
#endif /* Kyber */
 ...
 **************************************************/
CRS_STATIC size_t dil_r2sig2wire(unsigned char* sig, size_t sbytes, const polyvec_max* z,
 ...
 **************************************************/
CRS_STATIC size_t dil_r3sig2wire(unsigned char* sig, size_t sbytes, const unsigned char* chash,
                                 ...
                                 unsigned int dil_l)
{
    ...
    if ( sbytes < sb )
    {
        return 0;    /* TODO: report 'does not fit' */
    }
...
**************************************************/
CRS_STATIC int r2_wire2sig(polyvec_max* z, polyvec_max* h, poly* c, unsigned int dil_k,
...
**************************************************/
CRS_STATIC void dil_challenge(poly* c, const uint8_t mu[DIL_CRHBYTES], const polyvec_max* w1,
                              unsigned int k)
{
 ...
 **************************************************/
CRS_STATIC size_t r2_sign(const void* pKey, unsigned char* sig, size_t siglen, const uint8_t* m,
 ...
 * ...TODO: properly mapped ret.codes...
 **************************************************/
CRS_STATIC size_t r3_sign(const void* pKey, unsigned char* sig, size_t siglen, const uint8_t* m,
                          size_t mlen, const uint8_t* sk, size_t skbytes, unsigned int type)
{
    ...
    /* TODO: error return -> invalid/EINTERN param.set */
 ...
 **************************************************/
int r2_verify(const uint8_t* sig, size_t siglen, const uint8_t* m,
              size_t mlen, const uint8_t* pk, size_t pkbytes)
{
    ...
    {
        case 5:
#if !defined(NO_DILITHIUM_R2_3)
            unpack_pk5(rho, (polyvec5*)&pMat->t1, pk);
#endif
            break;

        case 6:
#if !defined(NO_DILITHIUM_R2_4)
            unpack_pk6(rho, (polyvec6*)&pMat->t1, pk);
#endif
            break;

        case 8:
#if !defined(NO_DILITHIUM_R2_5)
 ...
 * Returns >0 if signature could be verified correctly and 0 otherwise
 **************************************************/
CRS_STATIC int r3_verify(const void* pKey, const uint8_t* sig, size_t siglen, const uint8_t* m,
                         size_t mlen, const uint8_t* pk, size_t pkbytes)
{
    int           rc = 1;
    ...
    /* TODO: w1pb ... comparison SHOULD NOT fail */
    ...
    if ( rc == 1 )
    {
        ...
        {
            case 4:
                fail = !!spolyvec4_chknorm((const spolyvec4*)&pMat->z, gamma1 - beta);
                break;

            case 6:
                fail = !!spolyvec5_chknorm((const spolyvec5*)&pMat->z, gamma1 - beta);
                break;

            case 8:
                fail = !!spolyvec7_chknorm((const spolyvec7*)&pMat->z, gamma1 - beta);
                break;
        ...
        if ( rc == 1 )
        {
            ...
            shake256_finalize(&pMat->state);
            shake256_squeeze(mu, DIL_R3_CRHBYTES, &pMat->state);
            /* TODO: macro for all 1x/2x param invocations */
            ...
            switch ( K )
            {
                ...
                case 6:
                    ...
                    /* TODO: move K int use_hint; already const-propagated, but redundant
                        */
...
**************************************************/
CRS_STATIC void kpack_pk(uint8_t r[KYB_INDCPA_PUBLICKEYBYTES],
                         polyvec* pk,
                         const uint8_t seed[KYB_SYMBYTES])
{
    size_t i;

    kpolyvec_tobytes(r, pk);

    for (i = 0; i < KYB_SYMBYTES; i++)
    {
        r[i + KYB_POLYVECBYTES] = seed[i];
    }
}
...
**************************************************/
CRS_STATIC void kunpack_pk(polyvec* pk,
                           uint8_t seed[KYB_SYMBYTES],
                           const uint8_t packedpk[KYB_INDCPA_PUBLICKEYBYTES])
{
    size_t i;
...
**************************************************/
CRS_STATIC void kpack_sk(uint8_t r[KYB_INDCPA_SECRETKEYBYTES], polyvec* sk)
{
    kpolyvec_tobytes(r, sk);
}
...
**************************************************/
CRS_STATIC void kunpack_sk(polyvec* sk,
                           const uint8_t packedsk[KYB_INDCPA_SECRETKEYBYTES])
{
...
**************************************************/
CRS_STATIC void pack_ciphertext(uint8_t r[KYB_INDCPA_BYTES],
                                polyvec* b,
                                poly* v)
{
...
**************************************************/
CRS_STATIC void unpack_ciphertext(kpolyvec* b,
                                  kpoly* v,
 ...
 **************************************************/
CRS_STATIC unsigned int krej_uniform(int16_t* r, unsigned int len, const uint8_t* buf,
                                     unsigned int buflen)
{
    unsigned int ctr, pos;
    uint16_t     val;
 ...
 * Returns number of sampled 16-bit integers (at most len)
 **************************************************/
CRS_STATIC unsigned int r3_krej_uniform(int16_t* r, unsigned int len, const uint8_t* buf,
                                        unsigned int buflen)
{
    unsigned int ctr, pos;
    uint16_t     val0, val1;
...
//--------------------------------------
CRS_STATIC void kyber_shake128_absorb(Keccak_state* state, const uint8_t seed[KYB_SYMBYTES],
                                      uint8_t x, uint8_t y)
{
    ...
    crs_memclr(extseed, sizeof(extseed));
}

//--------------------------------------
CRS_STATIC void kyb_hash_g(unsigned char* h, const unsigned char* in, size_t ibytes)
{
    sha3_512(h, in, ibytes);
}

//--------------------------------------
CRS_STATIC void kyb_hash_h(unsigned char* h, const unsigned char* in, size_t ibytes)
{
...
CRS_STATIC void kyb_gen_matrix(kpolyvec_max* a, unsigned int k, const uint8_t seed[KYB_SYMBYTES],
                               int transposed)
{
...
CRS_STATIC void r3_kyb_gen_matrix(kpolyvec_max* a, unsigned int k, const uint8_t seed[KYB_SYMBYTES],
                                  int transposed)
{
 ...
 *                                      randomness
 **************************************************/
CRS_STATIC int indcpa_enc(uint8_t* c, size_t cbytes, const uint8_t m[KYB_INDCPA_MSGBYTES],
                          ...
                          unsigned int kyb_k)
{
    ...
    crs_memclr(seed, sizeof(seed));

    return rc;
}

CRS_STATIC int r3_indcpa_enc(uint8_t* c, size_t cbytes, const uint8_t m[KYB_INDCPA_MSGBYTES],
                             const uint8_t* pk, size_t pkbytes, const uint8_t coins[KYB_SYMBYTES],
                             unsigned int kyb_k)
{
    unsigned char seed[KYB_SYMBYTES] CRS_SENSITIVE;
 ...
 **************************************************/
CRS_STATIC int indcpa_dec(uint8_t m[KYB_INDCPA_MSGBYTES], const uint8_t* c, size_t cbytes,
...
CRS_STATIC int r3_indcpa_dec(uint8_t m[KYB_INDCPA_MSGBYTES], const uint8_t* c, size_t cbytes,
                             const uint8_t* sk, /* [KYB_INDCPA_SECRETKEYBYTES] */
                             unsigned int   kyb_k)
 ...
 **************************************************/
CRS_STATIC int dil_keygen(const void* pKey, unsigned char* prv, size_t prbytes, unsigned char* pub,
...
*/
CRS_STATIC int dilr3_keygen(const void* pKey, unsigned char* prv, size_t prbytes,
 ...
 **************************************************/
CRS_STATIC int kyb_keygen(unsigned char* prv, size_t prvbytes, unsigned char* pub, size_t* pbytes,
 ...
 **************************************************/
CRS_STATIC int kyb_kem1(unsigned char* ct, size_t cbytes, unsigned char* shrd, size_t* shbytes,
 ...
 **************************************************/
CRS_STATIC int kyb_kem2(unsigned char* shared, size_t sbytes, const unsigned char* ct,
...
#if !defined(NO_CRYSTALS_CIP) /*-- Kyber, en/decryption -------------------*/
/*------------------------------------*/
CRS_CRS_CRS_STATIC
/**/
int mlca_encrypt(unsigned char* cipher, size_t cbytes, const unsigned char* plain,
                 size_t plbytes, const unsigned char* pub, size_t pbytes, void* rng,
                 const unsigned char* algid, size_t ibytes)
{
    unsigned char encrseed[KYB_SYMBYTES] CRS_SENSITIVE;
    unsigned int  k = 0, type = 0;
    int           rc;

    if ( plbytes != KYB_INDCPA_MSGBYTES )
    {
        return MLCA_EKEYSIZE;
    }

    if ( randombytes(encrseed, sizeof(encrseed), rng) < sizeof(encrseed) )
    {
        return MLCA_ERNG;
    }
    ...
    if ( kyb_type2round(type) == 2 )
    {
        rc = indcpa_enc(cipher, cbytes, plain, pub, pbytes, encrseed, k);
    }
    else
    {
        rc = r3_indcpa_enc(cipher, cbytes, plain, pub, pbytes, encrseed, k);
    }

    crs_memclr(encrseed, sizeof(encrseed));
...
CRS_CRS_CRS_STATIC
/**/
int mlca_decrypt(unsigned char* plain, size_t plbytes, const unsigned char* cipher,
                 size_t cbytes, const unsigned char* prv, size_t pbytes,
                 const unsigned char* algid, size_t ibytes)
{
    unsigned int k = 0, type = 0;
    ...
    else if ( cipher && (cbytes == KYB_CIPHTXT3_BYTES) )
    {
        type = MLCA_ID_KYB3_R2;
    }
    else if ( cipher && (cbytes == KYB_CIPHTXT4_BYTES) )
    {
        type = MLCA_ID_KYB4_R2;
    }
    ...
    /* unlike KEM/2, priv-only key sufficient to decrypt */

    if ( pbytes < kyb_prv_wirebytes(k, 0) )
    {
        return MLCA_EKEYTYPE;
    }
    ...
    if ( !plain )
    {
        return KYB_INDCPA_MSGBYTES;    // size query
    }
    ...
    if ( kyb_type2round(type) == 2 )
    {
        return indcpa_dec(plain, cipher, cbytes, prv, k);
    }
    else
    {
        return r3_indcpa_dec(plain, cipher, cbytes, prv, k);
    }
}
#endif /*-----  /delimiter: Kyber, en/decryption ----------------------*/
...
/*--------------------------------------
    * TODO: prv.key-to-SPKI conversion
    */
CRS_CRS_CRS_STATIC
/**/
