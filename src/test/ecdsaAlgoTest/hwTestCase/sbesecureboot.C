...
/*                                                                        */
/* $Source: src/test/ecdsaAlgoTest/hwTestCase/sbesecureboot.C $           */
/*                                                                        */
                         ...
                         uint32_t *flag)
{
    ...
    //NIST Test vector .bin format. This format will repeat for 10 test
    //vectors.The test vector bnary is append into
    //P9_XIP_SECTION_SBE_SBH_FIRMWARE(.sbh_firmware xip section)
    //Message = 64
    //Pub Key = 132
    //Sig     = 132

    uint32_t size1 = getXipSize(P9_XIP_SECTION_SBE_SBH_FIRMWARE);
    unsigned char bin[size1];
    SBEV_INFO("Verify NIST signatures");
    SBEV_INFO(SBEV_FUNC "Secure Header:Start Offset: [0x%08X] Size: [0x%08X] ", getXipOffsetAbs(P9_XIP_SECTION_SBE_SBH_FIRMWARE), getXipSize(P9_XIP_SECTION_SBE_SBH_FIRMWARE));

    memcpy_byte(bin,(uint32_t*)getXipOffsetAbs(P9_XIP_SECTION_SBE_SBH_FIRMWARE),getXipSize(P9_XIP_SECTION_SBE_SBH_FIRMWARE));

    SBEV_INFO("Start addr [0x%08X]", bin);
    SBEV_INFO("Start addr + 64 [0x%08X]", bin[64]);

    uint32_t shift = 64+132+132;
    for (uint8_t i=0;i<10;i++)
    {
        if (ec_verify (bin+64+(i*shift), bin+(i*shift), bin+64+132+(i*shift))<1)
        {
            SBEV_INFO("NIST Signature verification failed for test vector %d",i+1);
        }
        else
        {
            SBEV_INFO("NIST Signature verification passed for test vector %d",i+1);
        }
    }

    //Halting PPE here to grab SBE traces and check for result.
    SBEV_INFO("Halting ppe");
    pk_halt();
