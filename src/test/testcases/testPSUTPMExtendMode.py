...
#
# $Source: src/test/testcases/testPSUTPMExtendMode.py $
#
...
'''
#------------------------------------------------------------------------------------------------------------------------------
# SBE side test data - Enter TPM Extend Mode, with 4096 milliseconds  timer
#------------------------------------------------------------------------------------------------------------------------------
'''
sbe_test_Enter_TPM_Extend_Mode = (
    #-----------------------------------------------------------------------------------------------------
    #   OP      Reg                               ValueToWrite         size    Test Expected Data       Description
    #-----------------------------------------------------------------------------------------------------
    ["write", reg.REG_MBOX0, "0000010100F0D602", 	 8, 	"None", 		"Writing to MBOX0 address"],
...
'''
#------------------------------------------------------------------------------------------------------------------------------
# SBE side test data - Exit TPM Extend Mode. (Stop Timer which was started in Enter TPM Extend Mode)
#------------------------------------------------------------------------------------------------------------------------------
'''
sbe_test_Exit_TPM_Extend_Mode = (
    #-----------------------------------------------------------------------------------------------------
    #   OP      Reg                               ValueToWrite         size    Test Expected Data       Description
    #-----------------------------------------------------------------------------------------------------
    ["write", reg.REG_MBOX0, "0000010200F0D602", 	 8, 	"None", 		"Writing to MBOX0 address"],
    ["write", reg.PSU_SBE_DOORBELL_REG_WO_OR, "8000000000000000", 	 8, 	"None", 		"Update SBE Doorbell register to interrupt SBE"],
    )
'''
...
def main():
    ...
    print("\n Excute TPM Extend Mode ...\n")

    '''
    Test Case 1
    '''
    print("\n  Test Enter TPM Extend Mode\n")
    # HOST->SBE data set execution
    regObj.ExecuteTestOp( testPSUUtil.simSbeObj, sbe_test_Enter_TPM_Extend_Mode )
    ...
    Test Case 2
    '''
    print("\n  Test Exit TPM Extend Mode\n")
    # HOST->SBE data set execution
    regObj.ExecuteTestOp( testPSUUtil.simSbeObj, sbe_test_Exit_TPM_Extend_Mode )
