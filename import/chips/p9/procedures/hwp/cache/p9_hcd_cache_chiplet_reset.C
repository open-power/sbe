    ...
    const fapi2::Target<fapi2::TARGET_TYPE_EQ>& i_target)
{
    ...
    FAPI_DBG("Init L3 Glitchless Mux Reset/Select via CLOCK_GRID_CTRL[0:3]");
    FAPI_TRY(putScom(i_target, EQ_PPM_CGCR_OR, MASK_OR(0, 4, 0x8)));

    FAPI_DBG("Init L2 Glitchless Mux Reset/Select via ANALOG_CLK_CTRL[16:19]");
    FAPI_TRY(putScom(i_target, EQ_QPPM_QACCR_OR, MASK_OR(16, 4, 0x8)));

    FAPI_DBG("Clear L3 Glitchless Mux Async Reset via CLOCK_GRID_CTRL[0]");
    FAPI_TRY(putScom(i_target, EQ_PPM_CGCR_CLEAR, MASK_SET(0)));

    FAPI_DBG("Clear L2 Glitchless Mux Async Reset via ANALOG_CLK_CTRL[16]");
    FAPI_TRY(putScom(i_target, EQ_QPPM_QACCR_CLEAR, MASK_SET(16)));
    ...
    FAPI_TRY(putScom(i_target, EQ_ERROR_REG, MASK_ALL));
