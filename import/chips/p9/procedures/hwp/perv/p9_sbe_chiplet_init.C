...
enum P9_SBE_CHIPLET_INIT_Private_Constants
{
    REGIONS_EXCEPT_VITAL = 0x7FF,
    SCAN_TYPES_EXCEPT_TIME_GPTR_REPR = 0xDCE
};

static fapi2::ReturnCode p9_sbe_chiplet_init_scan0_module_function(
                                      ...
                                      fapi2::Target<fapi2::TARGET_TYPE_PROC_CHIP>& i_target_chip)
{
    ...
    for (auto l_chplt_trgt : l_perv_functional_vector)
    {
        ...
        FAPI_INF("Call sbe_chiplet_init_scan0_module_function");
        FAPI_TRY(p9_sbe_chiplet_init_scan0_module_function(l_chplt_trgt));
    }

    FAPI_INF("Check  for XSTOP Bit");
    ...
    const fapi2::Target<fapi2::TARGET_TYPE_PERV>& i_target_chiplet)
{
    ...
    if ( l_read_reg )
    {
        FAPI_INF("run scan0 module for region except vital and scan types except GPTR, TIME, REPR");
