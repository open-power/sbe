...
//        p9_xip_tool <image> [-<flag> ...] delete [ <section0> ... <sectionN> ]
//        p9_xip_tool <image> [-<flag> ...] dis <section or .rings_summary>\n"//
//
...
const char* g_usage =
    ...
    "       p9_xip_tool <image> [-i<flag> ...] delete [ <section0> ... <sectionN> ]\n"
    "       p9_xip_tool <image> [-i<flag> ...] dis <section or .rings_summary>\n"//\n"
    "\n"
    "This simple application uses the P9-XIP image APIs to normalize, search\n"
    "update and edit P9-XIP images. This program encapsulates several commands\n"
    "in a common command framework which requires an image to operate on, a\n"
...
///
/// \param[in] i_item  A pointer to the "next" P9XipItem in the TOC.
///
...
///
static int pairRingNameAndAddr( void* i_image, const P9XipItem* i_item, void* io_pairing)
{
    int rc = 0, rcLoc = -1;
    P9XipItem tocItem;
    ...
    rcLoc = p9_xip_find( i_image, i_item->iv_id, &tocItem);

    if (rcLoc)
    {
        fprintf( stderr, "p9_xip_find() failed : %s\n", P9_XIP_ERROR_STRING(g_errorStrings, rcLoc));
                       ...
                       const char**   i_argv)
{
    ...
    // Determine P9-XIP section ID from the section name, e.g.
    //         .loader_text =>  P9_XIP_SECTION_LOADER_TEXT
    //         .text     =>  P9_XIP_SECTION_TEXT
    //         .rings    =>  P9_XIP_SECTION_RINGS
    if (strcmp(sectionName, ".header") == 0)
    {
        sectionId = P9_XIP_SECTION_HEADER;
    }
    else if (strcmp(sectionName, ".fixed") == 0)
    {
        sectionId = P9_XIP_SECTION_FIXED;
    }
    else if (strcmp(sectionName, ".fixed_toc") == 0)
    {
        sectionId = P9_XIP_SECTION_FIXED_TOC;
    }
    else if (strcmp(sectionName, ".loader_text") == 0)
    {
        sectionId = P9_XIP_SECTION_LOADER_TEXT;
    }
    else if (strcmp(sectionName, ".loader_data") == 0)
    {
        sectionId = P9_XIP_SECTION_LOADER_DATA;
    }
    else if (strcmp(sectionName, ".text") == 0)
    {
        sectionId = P9_XIP_SECTION_TEXT;
    }
    else if (strcmp(sectionName, ".data") == 0)
    {
        sectionId = P9_XIP_SECTION_DATA;
    }
    else if (strcmp(sectionName, ".toc") == 0)
    {
        sectionId = P9_XIP_SECTION_TOC;
    }
    else if (strcmp(sectionName, ".strings") == 0)
    {
        sectionId = P9_XIP_SECTION_STRINGS;
    }
    else if (strcmp(sectionName, ".base") == 0)
    {
        sectionId = P9_XIP_SECTION_BASE;
    }
    else if (strcmp(sectionName, ".baseloader") == 0)
    {
        sectionId = P9_XIP_SECTION_BASELOADER;
    }
    else if (strcmp(sectionName, ".rings") == 0)
    {
        sectionId = P9_XIP_SECTION_RINGS;
    }
    else if (strcmp(sectionName, ".rings_summary") == 0)
    {
        sectionId = P9_XIP_SECTION_RINGS;
        bSummary = 1;
    }
    ...
    {
        sectionId = P9_XIP_SECTION_OVERLAYS;
    }
    else if (strcmp(sectionName, ".hbbl") == 0)
    {
        sectionId = P9_XIP_SECTION_HBBL;
    }
    else
    {
    ...
    *disList = '\0'; // Make sure the buffer is NULL terminated (though probably not needed.)
    ...
    while ((uint64_t)nextBlock < (uint64_t)nextSection)
    {
        ...
        //
        if (sectionId == P9_XIP_SECTION_RINGS || sectionId == P9_XIP_SECTION_OVERLAYS)
        {
            ...
            //
            p9_xip_get_section( i_image, P9_XIP_SECTION_TOC, &hostSection);
            ...
            if (hostSection.iv_offset)
            {
                ...
                {
                    rc = p9_xip_map_toc( i_image, pairRingNameAndAddr, (void*)(&pairingInfo));
                ...
                {
                    fprintf( stderr, "ERROR : Error associated with p9_xip_map_toc().\n");
            ...
            else
            {
                ...
                //             p9_xip_search_fixed_toc()-like function). Maybe you can add a
        ...
        else  if (  sectionId == P9_XIP_SECTION_LOADER_TEXT ||
                    sectionId == P9_XIP_SECTION_TEXT)
        {
        ...
        if (sectionId == P9_XIP_SECTION_RINGS && bSummary)
        {
            //
